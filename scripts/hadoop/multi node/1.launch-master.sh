...
instance_req_id=$(aws ec2 request-spot-instances --spot-price $1 --instance-count 1 --type "one-time" --launch-specification file://master-specification.json --output text --query 'SpotInstanceRequests[*].SpotInstanceRequestId')
...
q
TEXT
ed generate-data-and-put-in-hdfs.sh<<TEXT
2d
2
i
ssh-keygen -f "/home/ubuntu/.ssh/known_hosts" -R $private_hostname
.
w
q
TEXT
